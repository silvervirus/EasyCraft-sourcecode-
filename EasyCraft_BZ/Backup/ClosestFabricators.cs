...
namespace EasyCraft
{
  public static class ClosestFabricators
  {
    ...
    private static FieldInfo baseCompField = typeof (GhostCrafter).GetField("baseComp", BindingFlags.Instance | BindingFlags.Public | BindingFlags.NonPublic);
    ...
    private static GhostCrafter[] Find()
    {
      ...
      if (Main.Settings.useStorage == NeighboringStorage.Inside && ((Player) Player.main).IsInside())
      {
        if (((Player) Player.main).IsInSub())
          ...
          ghostCrafterArray = (GhostCrafter[]) ((Component) currentInterior).GetComponentsInChildren<GhostCrafter>();
      }
      else if (Main.Settings.useStorage == NeighboringStorage.Range100)
      {
        foreach (LifepodDrop lifepodDrop in (LifepodDrop[]) Object.FindObjectsOfType<LifepodDrop>())
        {
          ...
          {
            foreach (GhostCrafter componentsInChild in (GhostCrafter[]) ((Component) lifepodDrop).GetComponentsInChildren<GhostCrafter>())
            {
      ...
      using (Dictionary<TechType, int>.Enumerator enumerator = crafted.GetEnumerator())
      {
        while (enumerator.MoveNext())
        {
          ...
          foreach (GhostCrafter fabricator in ClosestFabricators.fabricators)
          {
            ...
            {
              Base @base = (Base) ClosestFabricators.baseCompField.GetValue((object) fabricator);
              if (!Object.op_Implicit((Object) @base) || @base.IsPowered(((Component) fabricator).get_transform().get_position()))
              {
